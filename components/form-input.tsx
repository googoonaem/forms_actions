    ...
    <div className="w-[50%] max-w-[550px] mx-auto flex flex-col gap-0">
      <label className="input input-bordered rounded-full flex items-center gap-2 border-neutral-200">
        ...
        <input
          {...rest}
          name={name}
          className="hover:cursor-text focus:border-transparent focus:ring-0 grow outline-none placeholder:text-gray-300 border-none bg-transparent"
        />
      </label>
      {errors?.map((error, index) => (
        <span
          key={index}
          className="text-[#ee5e5e] font-medium leading-tight px-14"
        >
          {error}
        </span>
      ))}
